...
- "ticket" / "helpdesk" / "request to staff" / "report to admins" / "flag for staff"
  → The `/helpop <message>` flow ships as a **commented-out template** in `chatcontrol-bukkit/src/main/resources/rules/command.rs` (the block around `# Catch "/helpop <message>"` — every `#match`, `#then`, `#strip`, `#dont` line is prefixed with `#`, meaning INACTIVE). The `helpop` channel itself IS shipped active in `settings.yml` under `Channels.List.helpop` with format `chatcontrol-bukkit/src/main/resources/formats/helpop.yml`. To enable the `/helpop` shortcut command, the user must remove the leading `#` from every line of the two `#match ^[/]helpop` blocks in `command.rs` (the usage-help block and the `#match ^([/]helpop) (.*)` redirect block) and run `/chc reload`. Once enabled, staff either join the channel via `/channel join helpop` or run `/spy toggle chat helpop` to monitor passively, and reply via `/tell <player>` or `/channel send helpop <message>`. **Never tell a user `/helpop` works out of the box** — always tell them to uncomment the rule first.
